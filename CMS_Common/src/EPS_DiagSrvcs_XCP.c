	...
	else
	{
		...
		else
		{
			...
			for(I_Cnt_T_u08 = 0U; I_Cnt_T_u08 < D_XCP_USRDEFDAQ_BUFFLEN; I_Cnt_T_u08++)
			{
				...
				{
					if(t_DiagSrv_PIDtbl[t_Param_Uls_T_u08[I_Cnt_T_u08]].active == TRUE) /* PRQA S 492 */
					{
															...
															VAR(uint8, EPSDIAGSRVCS_VAR) DAQNum_Cnt_T_u08)
{
	...
	{
		if((t_DiagSrv_PIDtbl[t_PIDs_Uls_T_u16[I_Cnt_T_u08]].active == TRUE) && (t_DiagSrv_PIDtbl[t_PIDs_Uls_T_u16[I_Cnt_T_u08]].type == D_PIDTBL_VARTYPE)) /* PRQA S 492 */
		{
			TotalPIDBytes_Cnt_T_u08 += (uint8)t_DiagSrv_PIDtbl[t_PIDs_Uls_T_u16[I_Cnt_T_u08]].length; /* PRQA S 492 */
		}
		else
		{
			Resp_Uls_T_u08 = XcpErrorHandler(D_XCP_ERR_INACTIVESRV);
