 ...
 * %version:          13 %
 * %derived_by:       dzxvzg %
 * %date_modified:    Wed Oct 23 09:58:09 2013 %
 *---------------------------------------------------------------------------------------------------------------------
...
static inline void pbistSelfCheckInit(void);
...
static inline void setupFlash(void)
{
	...
	flashWREG->EEPROM_CONFIG = 0x00030002;
...
static inline void pbistSelfCheckInit(void)
{
	volatile uint8 i;
	/* Run a diagnostic check on the memory self-test controller */
	/* First set up the PBIST ROM clock as this clock frequency is limited to 90MHz... we are running at 80MHz  */
	systemREG1->MSTGCR |= 0x00000105;					/* PBIST ROM clock frequency = HCLK frequency /2 Disable memory self controller*/
	...
	pbistREG->RINFOL = 0x400;							/* Select RAM Group 11, which is actually the MIBADC1 */
	pbistREG->RINFOU = 0x00000000;						/* De-Select all RAM groups */
