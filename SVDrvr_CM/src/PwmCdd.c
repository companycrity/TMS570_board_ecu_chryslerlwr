...
* %version:          21 %
* %derived_by:       nzt9hv %
* %date_modified:    Fri Apr 19 18:06:50 2013 % 
******************************************************************************
...
STATIC VAR(uint16, PWMCDD_VAR_NOINIT) CDD_NextDCSmall_Cnt_M_u16;
...
#define d_PwmPrdMax_Cnt_u16             (d_PwmFreq_Hz_Cnt_u16 \
                                         /(uint32)(d_PWMFreqBase_Hz_u16 \
                                           - d_PWMFreqDither_Hz_u16))
#define d_PwmPrdMin_Cnt_u16             (d_PwmFreq_Hz_Cnt_u16 \
                                         /(uint32)(d_PWMFreqBase_Hz_u16 \
                                           + d_PWMFreqDither_Hz_u16))
#define d_PwmPrdRange_Cnt_u16           (d_PwmPrdMax_Cnt_u16 - (uint16)d_PwmPrdMin_Cnt_u16)
#define d_PwmPrdInitVal_Cnt_u16         ((uint16)(d_PWMClockFreq_Hz_u32 / (uint32)d_PWMFreqBase_Hz_u16))
...
STATIC FUNC(uint16, PWMCDD_CODE) DeadTimeComp_u16p0(uint32 DCPhsUnComp_Cnt_T_u24p8,  uint16 Phase_Rev_T_u0p16);
...
VAR(uint32, AUTOMATIC)    DCPhsUnComp_Cnt_T_u24p8[3];
...
VAR(uint16, AUTOMATIC) i;
	...
	/* Update Duty Cycle Small Data (Purposly at beginning of function to syncronize calculation with A/D based data) */
	CDD_DutyCycleSmall_Cnt_G_u16[CDD_CDDDataAccessBfr_Cnt_G_u16] = CDD_NextDCSmall_Cnt_M_u16;
    ...
    PWMPeriod_Cnt_T_u16 = (uint16)Limit_m(PWMPeriod_Cnt_T_u16, d_PwmPrdMin_Cnt_u16, d_PwmPrdMax_Cnt_u16);


    
	CDD_NextDCSmall_Cnt_M_u16 = d_MSBMask_Cnt_u16;
...
for(i = D_PHSAIDX_CNT_U16; i <= D_PHSCIDX_CNT_U16; i++)
{
    ...
    DCPhsUnComp_Cnt_T_u24p8[i] = (((uint32)ModIdxPhs_Cnt_T_u0p16[i] * (uint32)PWMPeriod_Cnt_T_u16) 
                               + (uint32)d_HalfPrec8_Cnt_u16)>>d_Scaler8_Cnt_u16;
    
    DCPhsComp_Cnt_T_u16p0[i] = DeadTimeComp_u16p0(DCPhsUnComp_Cnt_T_u24p8[i], Phase_Rev_T_u0p16);                                                                                                                                
...
if (TRUE == CDD_PwmDisable_Cnt_G_lgc[CDD_AppDataFbkPthAccessBfr_Cnt_G_u16])
{
    ...
    DCPhsComp_Cnt_T_u16p0[D_PHSAIDX_CNT_U16] = 0;
    DCPhsComp_Cnt_T_u16p0[D_PHSBIDX_CNT_U16] = 0;
    DCPhsComp_Cnt_T_u16p0[D_PHSCIDX_CNT_U16] = 0;
}
else
{
...
VAR(uint16, AUTOMATIC)  PWMPeriod_Cnt_T_u16;
    ...
    PWMPeriodFiltInp_Cnt_T_u16 = (uint16)((CDD_SeedPWMDither_Cnt_M_u16
                               * d_PwmPrdRange_Cnt_u16)>>d_Scaler16_Cnt_u16);
    ...
    PWMPeriod_Cnt_T_u16  = (uint16)((CDD_DitherFlt2SV_Cnt_M_u16 >> 
                                          d_FilterKdBits_Cnt_U16) + 
                                          d_PwmPrdMin_Cnt_u16);
...
/*****************************************************************************
 *  Name:           DeadTimeComp_u16p0
 *
 *  Description:    Applies the dead time compensation
 *
 *  Inputs:         u24p8_T: Uncompensated Phase Mod Index
 *                  u0p16_T:  Phase offset
 *  Outputs:        u16p0_T
 *****************************************************************************/
STATIC FUNC(uint16, PWMCDD_CODE) DeadTimeComp_u16p0(uint32 DCPhsUnComp_Cnt_T_u24p8, 
                                             uint16 Phase_Rev_T_u0p16)
{

    u16p0_T DCPhsComp_Cnt_T_u16p0;
    sint16  Polarity_Cnt_T_s16;
    u8p8_T  DeadTimeComp_Cnt_T_u8p8;

    if (DCPhsUnComp_Cnt_T_u24p8 > 0UL)
    {
        if ((Phase_Rev_T_u0p16 - d_30Deg_Rev_u0p16) <= d_180Deg_Rev_u0p16)
        {
        	Polarity_Cnt_T_s16 = CDD_MtrTrqCmdSign_Cnt_G_s16[CDD_AppDataFwdPthAccessBfr_Cnt_G_u16];
		}
		else
		{
			Polarity_Cnt_T_s16 = -CDD_MtrTrqCmdSign_Cnt_G_s16[CDD_AppDataFwdPthAccessBfr_Cnt_G_u16];
		...
		DeadTimeComp_Cnt_T_u8p8  = CDD_DeadTimeComp_Uls_G_u8p8
                                             [CDD_AppDataFwdPthAccessBfr_Cnt_G_u16];
	
        if (Polarity_Cnt_T_s16 >= 0)
        {
            DCPhsUnComp_Cnt_T_u24p8 += DeadTimeComp_Cnt_T_u8p8;
        }
        else
        {
            if (DCPhsUnComp_Cnt_T_u24p8 >= (uint32)DeadTimeComp_Cnt_T_u8p8)
            {
                DCPhsUnComp_Cnt_T_u24p8 -= DeadTimeComp_Cnt_T_u8p8;
            }
            else
            {
                DCPhsUnComp_Cnt_T_u24p8 = 0UL;
            }
        }
        
        DCPhsComp_Cnt_T_u16p0 = (uint16)((DCPhsUnComp_Cnt_T_u24p8 +
                                       (uint32)d_HalfPrec8_Cnt_u16)
                                    >> d_Scaler8_Cnt_u16);
                                    
        CDD_NextDCSmall_Cnt_M_u16 = Min_m(CDD_NextDCSmall_Cnt_M_u16, DCPhsComp_Cnt_T_u16p0);
     }
     else
     {
     	DCPhsComp_Cnt_T_u16p0 = (uint16)((DCPhsUnComp_Cnt_T_u24p8 +
