 ...
 * %version:          20 %
 * %derived_by:       czgng4 %
 * %date_modified:    Thu Jun 27 13:30:33 2013 %
 *---------------------------------------------------------------------------------------------------------------------
 ...
 *			    each applicaiton and also monitors for failure to write eeprom diagnostic
																...
																VAR(boolean, AUTOMATIC) CtrldDmpStsCmp_Cnt_T_lgc)
{
	...
	case RTE_MODE_StaMd_Mode_WARMINIT:	
		...
		break;
	case RTE_MODE_StaMd_Mode_OFF:	
		NvM_GetErrorStatus(0U, &NVMErrorStatus_Cnt_T_u8);
