...
; Metrics task hooks*
  osExtRef Metrics_TaskStart
  osExtRef Metrics_TaskEnd
...
osDispNoCheck
  ...
  ; Metrics Post Task Hooks
  bne    osL01                       ; Old task is running, so don't log termination

  mov    r3, sp                      ; save SVC SP
  mov    sp, r7                      ; run hook on task SP

  ;Call Metrics_TaskEnd to log the task termination.
  stmfd  sp!,{r0-r3,ip}
  mov    r0, r5                      ; Pass task ID as a parameter.
  blx    Metrics_TaskEnd
  ldmfd  sp!,{r0-r3,ip}

  mov    sp, r3                      ; restore SVC SP
...
osAfterMPUSetting
  ...
  ; Call Metrics pre task hook
  stmfd  sp!,{r0-r3,ip}
  mov    r0, r5
  blx    Metrics_TaskStart
  ldmfd  sp!,{r0-r3,ip}
...
RestoreContext
  ...
  ;Call Metrics pre task hook with task ID as parameter.
  stmfd  sp!,{r0-r3,ip}
  mov    r0, r5
  blx    Metrics_TaskStart
  ldmfd  sp!,{r0-r3,ip}

  ;Restore the stack OS stack pointer.
  mov    sp, r0
