 ...
 *
 *  Description: Header of BSW Scheduler for the NVRAM Manager 
 *
 ******************************************************************************
 *               C O P Y R I G H T
 ******************************************************************************
...
#define NVM_ACTIVATION_POINT_0   ACT_TIMEOUT_EVENT_10MS

extern const uint8 NVM_EXCLUSIVE_AREA_0;

#if defined( __SCHM__ )

SCHM_DEFINE_TIMER(NVM);

const uint8 NVM_EXCLUSIVE_AREA_0 = SCHM_EA_SUSPENDALLINTERRUPTS;

#  define SCHM_TIMER_INIT_NVM      SCHM_TIMER_INIT(NVM, NVM_ACTIVATION_POINT_0)
#  define SCHM_MAINFUNCTION_NVM    SCHM_MAINFUNCTION(NvM_MainFunction(), NVM, NVM_ACTIVATION_POINT_0)

Std_ReturnType SchM_ActMainFunction_NvM(uint8 activation)
{
   return SCHM_MAINFUNCTION_ACTIVATE(NVM, activation);
}

Std_ReturnType SchM_CancelMainFunction_NvM(uint8 activation)
{
   return SCHM_MAINFUNCTION_CANCEL(NVM);
}

void SchM_Enter_NvM(uint8 ExclusiveArea)
{
   SCHM_ENTER_EXCLUSIVE(ExclusiveArea);
}

void SchM_Exit_NvM(uint8 ExclusiveArea)
{
   SCHM_EXIT_EXCLUSIVE(ExclusiveArea);
}
#else /* __SCHM__ */

SCHM_EXTERN_TIMER(NVM);
